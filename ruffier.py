...
The sum of the three attempts at heart rate readings (before exertion, right after exertion,
...
7–8 9–10 11–12 13–14 15+ (only
for adolescents!)
perfect 6.4 and below 4.9 and below 3.4 and below 1.9 and below
0.4 and below
...
unsatisfactory 21 and above 19.5 and above 18 and above 16.5 and above
15 and above
...
the calculated Ruffier index and “unsatisfactory” level for the tested age, and produce a
result
'''
...
def ruffier_index(P1, P2, P3):
    ''' it returns the index value according to the three heart rate calculations for
    comparison with the table'''
    return (4 * (P1+P2+P3) - 200) / 10
def neud_level(age):
    ''' 
    the options with an age of less than 7 and with adults have to be processed
    separately,
    here we select the level “unsatisfactory” only within the table:
    for the age of 7, “unsatisfactory” is an index of 21, then onwards every 2 years it
    decreases by 1.5 until the level of 15 at age 15–16 
    '''
    norm_age = (min(age, 15) - 7) // 2 # every two years the from age seven difference is
                                        # one, all the way to age 15
    result = 21 - norm_age * 1.5 # every two years multiply the difference by 1.5, that's how
                                # the levels are arranged in the table
    return result
...
def ruffier_result(r_index, level):
    ...
    if r_index >= level:
        return 3
    return 4 
    # we end up here if the index is less than all the intermediate levels, in other
    # words, the tested circle.
...
def test(P1, P2, P3, age):
    ...
    if age < 7:
        ...
        # conversion of the numeric preparation level into text data
